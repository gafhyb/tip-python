...
def jwt_payload(content):
    import jwt
    return jwt.decode(content, verify=False)
...
def main(orig):
    ...
    def find():
        ...
        for pattern in patterns:
            ...
            if len(matches) > 0:
                for m in matches:
                    is_search &= 'search' not in pattern or ('search' in pattern and pattern['search'])
                    m_encoded = urllib.parse.quote_plus(f'{m}')
                    data = {'type': pattern['type'],
                            'label': pattern['label'].format(match=m, encoded=m_encoded)}
                    if 'generated' in pattern:
                        data['value'] = pattern['generated'].format(match=m, encoded=m_encoded)
                    elif 'actions' in pattern:
                        data['value'] = json.dumps(
                            [getattr(sys.modules[__name__], action)(m) for action in pattern['actions']]) if len(
                            pattern['actions']) > 1 else getattr(sys.modules[__name__], pattern['actions'][0])(m)
                    items.append(data)
        return is_search

    if find():
        items.append({'type': 'url', 'label': f'Google {input}', 'value': f'https://google.com/search?q={encoded}'})
